  ...
  async function handleDeleteSkill(event){
    ...
    async function updateChars(){
      try {
        ...
        user[charMode].forEach(s => characteristics = characteristics.filter(c=> c.de !== s.name))  
    ...
    <div className="pt-20 px-8 h-screen bg-slate-50">
        ...
        <Button onClick={handleInterestMode} color={`${charMode==='interests'? 'dark':'gray'}`}>
                Interessen
            </Button>
            <Button onClick={handleSkillMode} color={`${charMode === 'skills' ? 'dark':'gray'}`}>
                Skills
            </Button>
        </Button.Group>


        <p className="text-3xl font-semibold ml-2 mb-2" >{charMode==='interests'?'Interessen':'Skills'}</p>
        {/*ROW-1*/}
        <div className="mb-8">
            
            <div className=" border-solid border-2 border-gray-300 rounded-lg p-2 bg-slate-100  ">
                {user[charMode].map((s,index) => (
                <span key={index} id="badge-dismiss-green" className="inline-flex items-center px-2 py-1 mr-2 text-sm font-medium text-green-800 bg-green-100 rounded dark:bg-green-900 dark:text-green-300">
                    {s.name}
                    {/*flowbite badge*/}
                    <button onClick={charMode==='interests'?handleDeleteInterest:handleDeleteSkill} type="button" className="inline-flex items-center p-0.5 ml-2 text-sm text-green-400 bg-transparent rounded-sm hover:bg-green-200 hover:text-green-900 dark:hover:bg-green-800 dark:hover:text-green-300" data-dismiss-target="#badge-dismiss-green" aria-label="Remove">
        ...
        <p className="text-3xl font-semibold ml-2 mb-2" >{charMode==='interests'?'Interessen Pool':'Skills Pool'}</p>
        {/* Searchbar */}
        <form className="flex items-center mb-2">   
            ...
            <div className="relative w-full">
                ...
                <input onChange={handleSearchInput} type="text" id="simple-search" className="bg-slate-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Wähle Skills aus dem Skillpool..." required></input>
            </div>
        ...
        <div>
            <div className=" border-solid border-2 border-gray-300 rounded-lg p-2 bg-slate-100  ">
                {chars.map((c, index) => (
                    <>
                    {/*flowbite badge*/}
                    <button key={index}  onClick={charMode==='interests'?handleAddInterest:handleAddSkill}  className="bg-yellow-100 text-yellow-800 text-l font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-yellow-900 dark:text-yellow-300 hover:bg-green-200 hover:text-green-900">{c.de}</button>
                    </>
                ))}
